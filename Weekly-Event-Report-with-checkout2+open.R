...
data_web_otobus<-subset(data1,(data1$eventAction%in%c("viewJourneys","clickJourney","addToCart","checkout-1","checkout-2","purchase")&data1$deviceCategory!="tablet")|
                          ((data1$eventCategory%in%c("Bus Ticket Cancel")|data1$eventAction%in%c("My Bus Tickets"))&data1$deviceCategory!="tablet"&
                             data1$eventLabel%in%c("Clicked on Cancel Ticket","Clicked on Cancel Ticket from Cancel Pop-up","Clicked on Cancel Ticket from CancelOrOpen Pop-up",
                                                   "Clicked on Open Ticket from CancelOrOpen Pop-up","Clicked on Change Ticket","Clicked on Change Ticket from Change Pop-up",
                                                   "Clicked on Change Ticket from ChangeOrOpen Pop-up","Clicked on Open Ticket from ChangeOrOpen Pop-up"))|
...
for(i in 1:length(data_web_otobus$eventCategory)){
  if(data_web_otobus[i,"eventCategory"] == "Bus Ticket Cancel" || data_web_otobus[i,"eventCategory"] == "Membership"){
...
data_web_otobus_unique_gerekliler<-data_web_otobus_unique[c("deviceCategory", "viewJourneys", "clickJourney", "addToCart", "checkout-1", "checkout-2", "purchase", "PurchaseSuccess","Loggedin User Detected","Registered User Detected", "Successfull payment MasterPass",
                                                            ...
                                                            "Membership_Clicked on Open Ticket from CancelOrOpen Pop-up","Membership_Clicked on Open Ticket from ChangeOrOpen Pop-up")]
...
my_filter4_android<-dim_filter(dimension = "eventCategory",operator = "REGEXP",expressions = "My Tickets")
my_filter_clause_android<-filter_clause_ga4(list(my_filter1_android, my_filter2_android, my_filter3_android, my_filter4_android),operator = "OR")
...
data_android_otobus<-subset(data1_android,(data1_android$eventAction%in%c("viewJourneys","clickJourney","addToCart","checkout-1","checkout-2","purchase"))|
                              ((data1_android$eventCategory=="My Tickets"&data1_android$eventAction=="Bus Tickets")&data1_android$eventLabel%in%c("Clicked on Cancel","Clicked on Cancel Ticket from Cancel Pop-up","Clicked on Cancel Ticket from CancelOrOpen Pop-up",
                              "Clicked on Open Ticket from CancelOrOpen Pop-up","Clicked on Change","Clicked on Change Ticket from Change Pop-up",
                              "Clicked on Change Ticket from ChangeOrOpen Pop-up","Clicked on Open Ticket from ChangeOrOpen Pop-up"))|
...
data_android_otobus_unique_gerekliler<-data_android_otobus_unique[c("Ulasim_Yolu","Hafta","Kanal", "viewJourneys", "clickJourney", "addToCart", "checkout-1", "checkout-2","purchase", "PurchaseSuccess","Loggedin User Detected","Registered User Detected", "Successful payment MasterPass",
                                                                    "Clicked on Cancel","Clicked on Cancel Ticket from Cancel Pop-up","Clicked on Cancel Ticket from CancelOrOpen Pop-up",
                                                                    "Clicked on Open Ticket from CancelOrOpen Pop-up","Clicked on Change","Clicked on Change Ticket from Change Pop-up",
                                                                    "Clicked on Change Ticket from ChangeOrOpen Pop-up","Clicked on Open Ticket from ChangeOrOpen Pop-up")]
...
my_filter4_ios<-dim_filter(dimension = "eventCategory",operator = "REGEXP",expressions = "My Tickets")
my_filter_clause_ios<-filter_clause_ga4(list(my_filter1_ios, my_filter2_ios, my_filter3_ios, my_filter4_ios),operator = "OR")
...
filter(data1_android,eventCategory=="My Tickets")
##3.1. Ios otobüs
data_ios_otobus<-subset(data1_ios,(data1_ios$eventAction%in%c("viewJourneys","clickJourney","addToCart","checkout-1","checkout-2","purchase"))|
                          ((data1_ios$eventCategory=="My Tickets"&data1_ios$eventAction=="Bus Tickets")&data1_ios$eventLabel%in%c("Clicked on Cancel","Clicked on Cancel Ticket from Cancel Pop-up","Clicked on Cancel Ticket from CancelOrOpen Pop-up",
                          "Clicked on Open Ticket from CancelOrOpen Pop-up","Clicked on Change","Clicked on Change Ticket from Change Pop-up",
                          "Clicked on Change Ticket from ChangeOrOpen Pop-up","Clicked on Open Ticket from ChangeOrOpen Pop-up"))|
...
data_final[data_final$Ulasim_Yolu== "1. Otobus" & (data_final$Kanal %in% c("1. Mobile_Web","2. Desktop")) & is.na(data_final$KolayIslem_Iptal),"KolayIslem_Iptal"]=0
data_final[data_final$Ulasim_Yolu== "1. Otobus" & (data_final$Kanal %in% c("1. Mobile_Web","2. Desktop")) & is.na(data_final$KolayIslem_IptalPopup_Iptal),"KolayIslem_IptalPopup_Iptal"]=0
data_final[data_final$Ulasim_Yolu== "1. Otobus" & (data_final$Kanal %in% c("1. Mobile_Web","2. Desktop")) & is.na(data_final$KolayIslem_IptalyadaAcikPopup_Iptal),"KolayIslem_IptalyadaAcikPopup_Iptal"]=0
data_final[data_final$Ulasim_Yolu== "1. Otobus" & (data_final$Kanal %in% c("1. Mobile_Web","2. Desktop")) & is.na(data_final$KolayIslem_Degistir),"KolayIslem_Degistir"]=0
data_final[data_final$Ulasim_Yolu== "1. Otobus" & (data_final$Kanal %in% c("1. Mobile_Web","2. Desktop")) & is.na(data_final$KolayIslem_DegistirPopup_Degistir),"KolayIslem_DegistirPopup_Degistir"]=0
data_final[data_final$Ulasim_Yolu== "1. Otobus" & (data_final$Kanal %in% c("1. Mobile_Web","2. Desktop")) & is.na(data_final$KolayIslem_DegistiryadaAcikPopup_Degistir),"KolayIslem_DegistiryadaAcikPopup_Degistir"]=0
data_final[data_final$Ulasim_Yolu== "1. Otobus" & (data_final$Kanal %in% c("1. Mobile_Web","2. Desktop")) & is.na(data_final$KolayIslem_IptalyadaAcikPopup_Acik),"KolayIslem_IptalyadaAcikPopup_Acik"]=0
data_final[data_final$Ulasim_Yolu== "1. Otobus" & (data_final$Kanal %in% c("1. Mobile_Web","2. Desktop")) & is.na(data_final$KolayIslem_DegistiryadaAcikPopup_Acik),"KolayIslem_DegistiryadaAcikPopup_Acik"]=0

data_final[data_final$Ulasim_Yolu== "1. Otobus" & is.na(data_final$Biletlerim_Iptal),"Biletlerim_Iptal"]=0
data_final[data_final$Ulasim_Yolu== "1. Otobus" & is.na(data_final$Biletlerim_IptalPopup_Iptal),"Biletlerim_IptalPopup_Iptal"]=0
data_final[data_final$Ulasim_Yolu== "1. Otobus" & is.na(data_final$Biletlerim_IptalyadaAcikPopup_Iptal),"Biletlerim_IptalyadaAcikPopup_Iptal"]=0
data_final[data_final$Ulasim_Yolu== "1. Otobus" & is.na(data_final$Biletlerim_IptalyadaAcikPopup_Acik),"Biletlerim_IptalyadaAcikPopup_Acik"]=0
data_final[data_final$Ulasim_Yolu== "1. Otobus" & is.na(data_final$Biletlerim_Degistir),"Biletlerim_Degistir"]=0
data_final[data_final$Ulasim_Yolu== "1. Otobus" & is.na(data_final$Biletlerim_DegistirPopup_Degistir),"Biletlerim_DegistirPopup_Degistir"]=0
data_final[data_final$Ulasim_Yolu== "1. Otobus" & is.na(data_final$Biletlerim_DegistiryadaAcikPopup_Degistir),"Biletlerim_DegistiryadaAcikPopup_Degistir"]=0
data_final[data_final$Ulasim_Yolu== "1. Otobus" & is.na(data_final$Biletlerim_DegistiryadaAcikPopup_Acik),"Biletlerim_DegistiryadaAcikPopup_Acik"]=0 
